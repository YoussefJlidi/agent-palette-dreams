...
const Services = () => {
  const services = [{
    ...
    icon: Users,
    color: "primary",
    features: ["Analyse de vos besoins spécifiques", "Sélection rigoureuse des candidats", "Intégration personnalisée", "Suivi de performance"]
  }, {
    id: "formation",
    ...
    icon: BookOpen,
    color: "chart-2",
    features: ["Formations adaptées à votre secteur", "Ateliers pratiques réguliers", "Supports pédagogiques innovants", "Évaluation des acquis"]
  }, {
    id: "optimisation",
    ...
    icon: BarChart3,
    color: "chart-3",
    features: ["Tableaux de bord personnalisés", "Indicateurs clés de performance", "Analyses comparatives", "Recommandations d'amélioration continue"]
  }, {
    id: "coaching",
    ...
    icon: Clock,
    color: "chart-4",
    features: ["Séances individuelles régulières", "Objectifs clairs et mesurables", "Suivi des progrès", "Adaptation aux besoins individuels"]
  }];
  return <section id="services" className="py-20 bg-card/20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 animate-fade-in">
          <Badge variant="outline" className="px-4 py-1.5 mb-6 bg-card/50 backdrop-blur">
            Nos solutions
          </Badge>
          <h2 className="heading-lg mb-4">Confiez vos performances aux agents IA</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <Accordion type="single" collapsible className="w-full">
              {services.slice(0, 2).map(service => <AccordionItem key={service.id} value={service.id} className="border border-border/40 rounded-lg mb-4 overflow-hidden bg-card/50 backdrop-blur-sm shadow-sm hover:shadow-md transition-all">
          ...
          <div className="space-y-4">
            <Accordion type="single" collapsible className="w-full">
              {services.slice(2, 4).map(service => <AccordionItem key={service.id} value={service.id} className="border border-border/40 rounded-lg mb-4 overflow-hidden bg-card/50 backdrop-blur-sm shadow-sm hover:shadow-md transition-all">
                  <AccordionTrigger className="px-6 py-4 flex items-center hover:bg-accent/50 hover:no-underline">
                    <div className="flex items-center">
                      <div className={`w-10 h-10 rounded-md bg-${service.color}/10 flex items-center justify-center mr-4`}>
                        <service.icon className={`text-${service.color} h-5 w-5`} />
